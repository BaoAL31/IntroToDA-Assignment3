...
class Net(nn.Module):
    def __init__(self, class_counts):
        super().__init__()
        self.embeddings = nn.ModuleList([nn.Embedding(num_embeddings=class_count, embedding_dim=10) for class_count in class_counts])
        self.ln1 = nn.Linear(96, 64)
...
loss_fn = nn.BCEWithLogitsLoss(pos_weight=torch.tensor(8.9))
