...
class Net(nn.Module):
    def __init__(self, class_counts):
        ...
        self.emb_lin = nn.Linear(20, 1)
        self.bn1 = nn.BatchNorm1d(16)
        self.ln1 = nn.Linear(3, 32)
    ...
    def forward(self, x1, x2):
        ...
        x = [self.emb_lin(i) for i in x_cat]
        x = torch.cat(x, 1)
        # print(x.shape)
        # x = self.emb_drop(x)
        x = F.relu(self.ln1(x))
...
def preprocessing(df):
    ...
    df.drop(labels=['marital_status', 'LOSgroupNum', 'AdmitDiagnosis', 'AdmitProcedure', 'gender', 'religion', 'ethnicity'], axis=1, inplace=True)

    categorical_features = ['admit_type', 'admit_location', 'insurance']
